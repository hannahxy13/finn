...
def make_vivado_platform(cfg):
    xsa_platform_tcl = "../../src/vivado/xsa_platform_classic.tcl"
    board_name = cfg.board.lower()
    platform_custom = board_name + "_thin"
    device_name = cfg.fpga_part
    board_ver = 3.1
    ap_clk_mhz = 1 / (cfg.synth_clk_period_ns / 1000)
    ap_clk_2x_mhz = 2 * ap_clk_mhz

    if os.path.exists("instr_wrap_build/vivado"):
        shutil.rmtree("instr_wrap_build/vivado")
    os.makedirs("instr_wrap_build/vivado")
    build_command = "cd instr_wrap_build/vivado && \
        vivado -mode batch -source %s -tclargs %s %s %s %s %s %s" % (
            xsa_platform_tcl,
            board_name,
            platform_custom,
            device_name,
    ...
    result = subprocess.call(build_command, shell=True)
    


def make_full_impl(cfg, ila_en, double_pumped):
    platform_custom = cfg.board.lower() + "_thin"
    target = "hw"
    xsa_platform = "../vivado/vivado_build/xsa_platform/%s.xsa" % platform_custom

    """
    if ila_en == 1:
        if doubled_pumped == 1:
            system_cfg = "src/2x/system.cfg src/2x/ila_0_bd.cfg"
        elif doubled_pumped == 0:
            system_cfg = "src/1x/system.cfg src/1x/ila_0_bd.cfg"
    if ila_en == 0:
        if doubled_pumped == 1:
    ...
    system_cfg = "../../src/vitis/system.cfg"

    if os.path.exists("instr_wrap_build/vitis"):
        shutil.rmtree("instr_wrap_build/vitis")
    os.makedirs("instr_wrap_build/vitis")

    xo = "../../%s/xo/finn_design.xo ../../%s/xo/instrumentation_wrapper.xo" % (cfg.output_dir, cfg.output_dir)
    output_xsa = platform_custom + ".xsa"

    build_command = "cd instr_wrap_build/vitis && \
        v++ --link --debug --save-temps \
        --target %s --optimize 3 --platform %s --config %s \
        %s --output %s 2>&1 | tee vpp_link.log" % (
            target,
            xsa_platform,
            system_cfg,
            xo,
...
def make_run_instr_wrap(cfg):
    platform_custom = cfg.board.lower() + "_thin"
    hw_server_host = "xirdcglab52"
    hw_server_port = 3121
    ila_en = 0
    ap_clk_mhz = 100

    shutil.copy(
        "instr_wrap_build/vitis/_x/link/vivado/vpl/prj/prj.runs/impl_1/%s_wrapper.pdi" % platform_custom,
        "instr_wrap_build/vitis"
    )

    build_command = "cd instr_wrap_build/vitis && \
        vivado _x/link/vivado/vpl/prj/prj.xpr -mode batch -source ../../src/vitis/export_xsa_program_device.tcl -tclargs %s %s %s %s" % (
            platform_custom,
            hw_server_host,
            hw_server_port,
            ila_en
    ...
    run_instr_wrap_sh = "./run_instr_wrap.sh"
    run_instr_wrap_command = "cd instr_wrap_build/vitis && \
        xsct ../../src/vitis/xsct_run_instr_wrap.tcl %s %s %s %s" % (
            platform_custom,
            ap_clk_mhz,
            hw_server_host,
            hw_server_port
    ...
    with open(run_instr_wrap_sh, "w") as f:
        f.write(run_instr_wrap_command)
